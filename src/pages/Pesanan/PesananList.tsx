        ...
        <div className="p-4 space-y-6">
            <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                    ...
                    <p className="text-gray-400 text-xs mt-0.5">Kelola transaksi pesanan</p>
                </div>
            ...
            <div className="space-y-3">
                {isLoading ? (
                    <p className="text-center text-gray-400 py-8">Memuat pesanan...</p>
                ) : orders.length === 0 ? (
                    <p className="text-center text-gray-500 py-8 text-sm">Belum ada pesanan.</p>
                ) : (
                    orders.map(order => (
                            ...
                            <Card className="hover:border-gray-700 transition-colors">
                                <div className="flex justify-between items-start mb-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h4 className="font-bold text-gray-100">{order.no_pesanan}</h4>
                                    </div>
                                ...
                                <div className="text-sm text-gray-300">
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">{new Date(order.tanggal).toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                                </div>

                                {order.order_details && order.order_details.length > 0 && (
                                    <div className="mt-3 pt-3 border-t border-gray-800 flex justify-between items-center">
                                        <span className="text-xs text-gray-500">{order.order_details.length} item(s)</span>
